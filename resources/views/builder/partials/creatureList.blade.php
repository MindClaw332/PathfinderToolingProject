@foreach($chosenCreatures as $index => $creature)
    ...
    <div class="flex flex-row justify-between block" id="creature-{{ $index }}">
        ...
        <button onclick="removeCreature({{ $index }}); calculateXP()" class="w-2/12 m-2 p-1 text-sm rounded-lg bg-red-800" type="button">
            Delete
        </button>
    </div>
    <!-- edit form creatures -->
    <form class="flex flex-row justify-between hidden" id="edit-{{ $index }}">
        ...
        <div class="flex flex-row w-1/3 gap-2 justify-end">
            <button onclick="updateCreature({{ $index }}); calculateXP()" class="w-1/2 mt-2 mb-2 p-1 text-sm rounded-lg bg-accent" type="button">
                Edit
            </button>
